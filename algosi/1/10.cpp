...
struct Node {
    int xKey;
    vector<Point> yPoints; //точки поддерева, отсортированные по y
    Node* left;
    Node* right;
};
...
void queryRangeTree(Node* root, int x1, int x2, int y1, int y2, vector<Point> &result){
    ...
    if(root->xKey < x1){
        ...
        auto lower = lower_bound(vecY.begin(), vecY.end(), Point{0, y1}, [](const Point &a, const Point &b){
            return a.y < b.y;
        });

        auto upper = upper_bound(vecY.begin(), vecY.end(), Point{0, y2}, [](const Point &a, const Point &b){
            return a.y < b.y;
        });
...
int main(){
    ...
    answer.erase(unique(answer.begin(), answer.end(), [](const Point &a, const Point &b){
        return (a.x == b.x && a.y == b.y);
    }), answer.end());

    cout << "points in [" << x1 << ".." << x2 << "] x [" << y1 << ".." << y2 << "]:\n";
